...
async function checkConfigStatus() {
    try {
        ...
        const data = await response.json();
        if (!data.configured) { window.location.href = '/settings.html'; return false; }
        return true;
    } catch { return false; }
}
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    document.querySelectorAll('.close').forEach(closeBtn => {
        closeBtn.addEventListener('click', function() { const modal = this.closest('.modal'); if (modal) modal.classList.remove('active'); });
    });
    document.querySelectorAll('.modal-backdrop').forEach(backdrop => {
        backdrop.addEventListener('click', function() { const modal = this.closest('.modal'); if (modal) modal.classList.remove('active'); });
    });
});
...
async function loadRecords() {
    try {
        ...
        applyFilters();
        showLoading(false);
    } catch (error) { showError(`Failed to load records: ${error.message}`); showLoading(false); }
}
...
async function handleEditRecord(e) {
    ...
    try {
        ...
        const response = await fetch(`${API_BASE_URL}/records/${type}/${originalName}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
        const data = await response.json();
...
async function deleteRecord(name, type) {
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/records/${type}/${name}`, { method: 'DELETE' });
        const data = await response.json();
